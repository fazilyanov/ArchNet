...
namespace WebArchiveR6
{
    public class faListNtd_OLD
    {
        ...
        public int EditFormTabWidth { get; set; }
                    ...
                    if (ws != null)
                    {
                        ...
                        foreach (faField fld in MainCursor.Fields)
                            if (fld.View.Visible)
                            {
                                row = 1;
                                i++;
                                ws.Cells[row, i].Value = fld.View.Caption;
            ...
            else if (_act == "export_csv" && _mode != "")
            {
                ...
                if (rowtop < tr)
                {
                    ...
                    foreach (DataRow r in dt.Rows)
                    {
                        foreach (faField fld in MainCursor.Fields)
                            if (fld.View.Visible)
                                _csv += r[fld.LookUp.Key != "" ? fld.Data.FieldName + "_" + fld.LookUp.Field + "_text" : fld.Data.FieldName].ToString().Replace(";", "") + ";";
                        _csv += "\n";
                    }
                    sw.Write(_csv);
                    sw.Close();
        ...
        protected DataTable GetData(faCursor cur, int _offset, int _limit, string _ordercolumn, string _orderdir, out int _totalrow)
        {
            ...
            HttpContext.Current.Session["last_sql"] = _last_sql.Length > 20000 ? _last_sql.Substring(2000) : _last_sql;
        ...
        protected DataTable GetDataList(faCursor cur)
        {
            ...
            if (HttpContext.Current.Session[_s] == null)
            {
                ...
                if (_mode == "nvfob" && cur.SesCurName == BaseName + "_docversion_cursor")
                {
                    ...
                    newrow["id"] = -1;
                    newrow["status"] = 1;
                    //newrow["main"] = improw["main"];
                    //newrow["status"] = improw["status"];
                    //newrow["id_status"] = improw["id_status"];
                    //newrow["barcode"] = improw["barcode"];
                    //newrow["file"] = improw["file"];
                    //newrow["date_trans"] = improw["date_trans"];
                    //newrow["id_source"] = improw["id_source"];
                    //newrow["id_source_name_text"] = improw["id_source_name_text"];
        ...
        /// </summary>
        public virtual string SetListRowDoubleClick(faCursor cur)
        {
            string ret = "";
        ...
        public virtual string GetColumnViewString(faCursor cur)
        {
            return faFunc.GetUserSetting(cur.Alias + (Page != faPage.none ? "_" + Page : "") + "_column_view");
        }
        ...
        public void PrepareJQGrid(out JQGrid jqGrid)
        {
            ...
            jqGrid.SortSettings.InitialSortDirection = (cur.SortDirection.ToLower() == "desc" ? Trirand.Web.UI.WebControls.SortDirection.Desc : Trirand.Web.UI.WebControls.SortDirection.Asc);
            ...
            jqGrid.ClientSideEvents.RowDoubleClick = SetListRowDoubleClick(cur);
            ...
            if (cur.EnableExportButton && Page != faPage.select)
            {
            ...
            //        _c[i].HeaderText = cur.Fields[i].View.Caption != "" ? cur.Fields[i].View.Caption : cur.Fields[i].View.Hint;
            ...
            foreach (faField fld in cur.Fields)
            {
                if (fld.Edit.Control != faControl.TextArea)
                {
                    ...
                    _c[i].HeaderText = fld.View.Caption != "" ? fld.View.Caption : fld.View.Hint;
            ...
            string column_view_string = GetColumnViewString(cur);
                    ...
                    if (fld.Filter.Enable)
                    {
                        ...
                        switch (fld.Filter.Control)
                        {
                            ...
                            case faControl.TextBoxFullSearch:
                                ...
                                _ret += "               $('" + n + "_cond' ).val('*');cld('#clear_" + n + "');\" title=\"" + (fld.View.Hint != "" ? fld.View.Hint : fld.View.Caption) + "\">" + (fld.Filter.Caption != "" ? fld.Filter.Caption : fld.View.Caption);
                                _ret += "           </button>";
                            ...
                            case faControl.TextBoxInteger:
                                ...
                                _ret += "               cld('#clear_" + n + "');\" title=\"" + (fld.View.Hint != "" ? fld.View.Hint : fld.View.Caption) + "\">" + (fld.Filter.Caption != "" ? fld.Filter.Caption : fld.View.Caption);
                                _ret += "           </button>";
                                _ret += "           <button type=\"button\" class=\"btn btn-xs btn-default dropdown-toggle\" data-toggle=\"dropdown\">";
                                _ret += "               <span class=\"caret\"></span><span class=\"sr-only\">Toggle Dropdown</span>";
                                _ret += "           </button>";
                            ...
                            case faControl.TextBoxNumber:
                                ...
                                _ret += "       <div class=\"input-group-btn\">";
                                _ret += "           <button type=\"button\" id=\"clear_" + n + "\" class=\"btn btn-xs btn-default\" ";
                                _ret += "               style=\"width: 100px;\" onclick=\"$('#cph_" + n + "_begin' ).val(''); $('#cph_" + n + "_end' ).val('');cld('#clear_" + n + "');\" title=\"" + (fld.View.Hint != "" ? fld.View.Hint : fld.View.Caption) + "\">";
                                _ret += "               " + (fld.Filter.Caption != "" ? fld.Filter.Caption : fld.View.Caption) + "</button>";
                                _ret += "       </div>";
        ...
        public string GenerateFormJS()
        {
            ...
            _js += " } var wf = " + this.EditFormWidth + ";var wt = wf - " + this.EditFormTabWidth + "; jQuery('#mainform').width(wf);jQuery('#tabs').width(wt);$('#tabs').find('textarea').width(wt-16);}).trigger('resize');\n";
        ...
        public virtual string SetCurRowDoubleClick(string _cn)
        {
            return "ViewRow" + _cn;
        }
        ...
        public void PrepareCursorJQGrid(string _cn, out JQGrid jqGrid)
        {
            ...
            jqGrid.Width = this.EditFormWidth - this.EditFormTabWidth - 2;//386
            ...
            else
            {
                ...
                JSReadyList.Add(jqGrid.ID + "_width_pager", "$('#cph_" + jqGrid.ID + "_pager_left').width(350);");
            }
            jqGrid.SortSettings.InitialSortColumn = cur.SortColumn;
            ...
            jqGrid.ClientSideEvents.RowDoubleClick = SetCurRowDoubleClick(_cn);
            ...
            foreach (var fld in cur.Fields)
            {
                ...
                _c.HeaderText = fld.View.Caption;
            ...
            jqGrid.RowEditing += (s, e) =>
            {
                try
                {
                    ...
                    foreach (var fld in cur.Fields)
                    {
                        ...
                        if (showerror)
                        {
                            (s as JQGrid).ShowEditValidationMessage(String.Format(faEdit.MsgMustFill, fld.View.Caption));
                            return;
                        }
                    }
                            ...
                            foreach (DataRow r in dt.Rows)
                            {
                                ...
                                switch (fld.Edit.Control)
                                {
                                    case faControl.TextBox:
                                        ...
                                        {
                                            (s as JQGrid).ShowEditValidationMessage(String.Format(faEdit.MsgDublicate, fld.View.Caption, r["id"].ToString()));
                                            return;
                                        }
                                        break;
                            ...
                            switch (fld.Edit.Control)
                            {
                                ...
                                case faControl.URL:
                                    if (fld.Edit.Max > 0 && value.Length > fld.Edit.Max)
                                        error = String.Format(faEdit.MsgMaxText, fld.View.Caption, fld.Edit.Max);
                                    else if (fld.Edit.Min > 0 && value.Length < fld.Edit.Min)
                                        error = String.Format(faEdit.MsgMinText, fld.View.Caption, fld.Edit.Min);
                                ...
                                case faControl.TextBoxNumber:
                                    if (Decimal.TryParse(value == "" ? "0" : value, out _decimal))
                                    {
                                        if (fld.Edit.Max > 0 && _decimal > fld.Edit.Max)
                                            error = String.Format(faEdit.MsgMaxInt, fld.View.Caption, fld.Edit.Max);
                                        else if (fld.Edit.Min > 0 && _decimal > 0 && _decimal < fld.Edit.Min)
                                            error = String.Format(faEdit.MsgMinInt, fld.View.Caption, fld.Edit.Min);
                                        row[fld.Data.FieldName] = _decimal;
                                    }
                                    else
                                        error = String.Format(faEdit.MsgBadFormat, fld.View.Caption);
                                    break;
                ...
                if (dt.Rows.Count != 0 || _act == "add" || _act == "copy" || _act == "copybase")
                {
                    ...
                    foreach (var fld in cur.Fields)
                    {
                        ...
                        if (fld.Edit.Visible && fld.Edit.Control != faControl.TextArea)
                        {
                            ...
                            _ret += "   <label style=\"width:"+this.EditFormLabelWidth.ToString()+"px;float:left;\">" + fld.View.Caption + (fld.Edit.Required ? " *" : "") + "</label>";
                        ...
                        foreach (string k in list)
                        {
                            ...
                            foreach (var fld in Cursors[k].Fields)
                            {
                                if (fld.Edit.Visible)
                                {
                                    ...
                                    _ret += "<div class=\"input-group\" style=\"margin-top:8px;\"> ";
                                    _ret += "   <label style=\"width:110px;float:left;\">" + fld.View.Caption + "</label>";
                    ...
                    foreach (var fld in cur.Fields)
                    {
                        if (fld.Edit.Visible && fld.Edit.Control == faControl.TextArea)
                        {
                            _tabs += "<li><a href=\"#tab" + fld.Data.FieldName + "\">" + fld.View.Caption + "</a></li>";
...
if (_act == "copybase")
            ...
            #region Проверяем есть ли галочки в версиях - для установки статуса
            ...
            #endregion Проверяем есть ли галочки в версиях - для установки статуса
            ...
            foreach (var fld in MainCursor.Fields)
            {
                ...
                if (showerror)
                {
                    result = String.Format(faEdit.MsgMustFill, fld.View.Caption);
                    return false;
                }
            ...
            foreach (var fld in MainCursor.Fields)
            {
                if (fld.Edit.Visible && fld.Edit.Unique)
                {
                    ...
                    if (!(res_u is DBNull || res_u == null))
                    {
                        result = String.Format(faEdit.MsgDublicate, fld.View.Caption, (int)res_u);
                        return false;
                    }
                }
            }
            ...
            #region CheckArchiveFieldAndStatus

            ////
            ////if (list.Page != faPage.empl && list.Page != faPage.tech) {
            //if (MainCursor.Alias ==  "zao_stg_archive" )
            //{
            //    if (Page != faPage.ord && Page != faPage.empl && Page != faPage.tech)
            //    {
            //        showerror = false;
            //        foreach (var fld in MainCursor.Fields)
            //        {
            ...
            //                case "id_prjcode":
            //                case "id_perf":
            //                    if (fld.Edit.Value == "0" || fld.Edit.Value == "-1" || fld.Edit.ValueText == "") showerror = true;
            //                    break;
            //            }
            //        }
            //        if (showerror)
            //        {
            //            result = faEdit.MsgMustFillForStatus;
            //            return false;
            //        }
            //    }
            //}

            #endregion CheckArchiveFieldAndStatus

            #region Unique Archive
            ...
            #endregion Unique Archive

            #region ChangeDocType
            ...
            try
            {
                ...
                if (CursorCount > 0)
                {
                    ...
                    foreach (string k in list)
                    {
                        ...
                        //if (_act == "copybase")
                        //{
                        //    _oldalias = Cursors[k].Alias;
                        //    Cursors[k].Alias = _frombase + "_" + Cursors[k].Alias.Split('_')[1].ToString();
                        //}
                        ...
                        #region Проверка на заплненость статуса, при создании из комплектов
                        ...
                        #endregion Проверка на заплненость статуса, при создании из комплектов
