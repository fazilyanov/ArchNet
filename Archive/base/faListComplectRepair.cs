...
{
    public class faListComplectRepair
    {
        #region CONST
        ...
        private string[] act_list = new string[] { "add", "view", "del", "copy", "file", "select" };
        public string _act = "";
        public string _id = "";
        public string _from = "";
        public string _mode = "";
        //string _id_base = ""; // комплекты общие а попадать нужно в разные базы.. храним тут id
        //string _id_base_name = ""; // комплекты общие а попадать нужно в разные базы.. храним тут имя базы eng
        ...
        protected DataTable GetData(faCursor cur, int _offset, int _limit, string _ordercolumn, string _orderdir, out int _totalrow)
        {
            ...
            if (RouteName == "complectrepairlist")
            {
                foreach (DataRow row in dt.Rows)
                {
                    if (row["barcode"].ToString() != "" && row["id_base"].ToString() != "0")
                    {
                        string _sql1 = "SELECT a.id_archive,a.[file] FROM [dbo].[" + row["base"].ToString() + "_docversion] as a where a.barcode=@barcode and a.del=0 order by a.id desc";
                        string _sql2 = "SELECT a.[file] FROM [dbo].[_complectnew_list] as a where a.barcode=@barcode and a.del=0 order by a.id desc";
                        // dt.Columns.Add("cb", Type.GetType("string"));
                        ...
                        else
                        {
                            ...
                            sqlDataAdapter.Fill(res2);
                            if (res2.Rows.Count > 0) row["file"] = res2.Rows[0]["file"];
                        }
                    }
                }
            }
        ...
        protected DataTable GetDataList(faCursor cur)
        {
            ...
            if (HttpContext.Current.Session[_s] == null)
            {
                ...
                // Достаем путь до файла в комплекте  движения документов
                if (cur.SesCurName == "_complect_repair_list_cursor")
                {
                    SqlConnection conn = new SqlConnection(Properties.Settings.Default.constr);
                    conn.Open();
                    string cb = "";
                    ...
                    if (_id != "0")
                    {
                        cmd.CommandText = "SELECT a.id_base FROM [dbo].[_complect_repair] as a where a.id=@id and a.del=0";
                    ...
                    string _sql1 =
                        "SELECT a.id_archive,a.[file], d.name as doctree,b.date_doc, b.num_doc, f.name as frm, dt.name as doctype " +
                        "FROM [dbo].[" + cb + "_docversion] as a " +
                        "Left Join [dbo].[" + cb + "_archive] as b ON a.id_archive=b.id " +
                        "Left Join [dbo]._doctree d on b.id_doctree=d.id " +
                        "Left Join [dbo]._frm f on b.id_frm_contr=f.id " +
                        "Left Join [dbo]._doctype dt on b.id_doctype=dt.id " +
                        "Where a.barcode=@barcode and a.del=0 order by a.id desc";
                    string _sql2 = "SELECT a.[file] FROM [dbo].[_complectnew_list] as a where a.barcode=@barcode and a.del=0 order by a.id desc";
                    // dt.Columns.Add("cb", Type.GetType("string"));

                    foreach (DataRow row in dt.Rows)
                    {
                        if (row["barcode"].ToString() != "" && row["barcode"].ToString() != "0")
                        {
                            cmd.Parameters.Clear();
                            cmd.Parameters.AddWithValue("barcode", row["barcode"].ToString());
                            cmd.CommandText = _sql1;
        ...
        public void PrepareJQGrid(out JQGrid jqGrid)
        {
            ...
            if (cur.EnableFileButton)
            {
                ...
                JSFunctionList.Add("GoToFile_" + cur.Alias + "()",
                    ...
                    "   var f = row.file;" +
                    "   if (f!='') window.open('/Archive/GetFile.aspx?id='+id +'&b=' + row.base +'&f=' + f + '&k='+$.md5(id));" +
            ...
            if (RouteName == "complectrepairlist")
            {
                ...
                jqBtnViewComplect.ToolTip = "Открыть соответствующий записи комплект движения документов";
                ...
                JSFunctionList.Add("GoToViewComplect_" + cur.Alias + "()",
                    ...
                    "if (id>0) {" +
                    "   var row = grid.getRowData(id);" +
                    "   var f = row.id_complect_repair;" +
                    "   window.open('" + "/complectrepair/?id='+f+'&act=view');" +
                    "}else alert('Выберите запись');  return false;");
                    ...
                    if (f.Fields[i].Filter.Enable)
                    {
                        ...
                        switch (f.Fields[i].Filter.Control)
                        {
                            ...
                            case faControl.DateTimePicker:
                                ...
                                _ret += "$('#cph_" + _n + "_begin').mask('99.99.9999 00:00',{placeholder:'дд.мм.гггг мм:сс'});";
                                _ret += "$('#cph_" + _n + "_end').mask('99.99.9999 00:00',{placeholder:'дд.мм.гггг мм:сс'});";
                                break;
        ...
        public string RenderEditForm(string alert)
        {
            ...
            if (act_list.Contains(_act) && int.TryParse(_id, out int_value))
            {
                ...
                if (dt.Rows.Count != 0 || _act == "add" || _act == "copy")
                {
                    ...
                    foreach (var fld in cur.Fields)
                    {
                        ...
                        if (fld.Edit.Visible && fld.Edit.Control != faControl.TextArea)
                        {
                            ...
                            switch (fld.Edit.Control)
                            {
                                ...
                                case faControl.DatePicker:
                                    fld.Edit.Value = (RequestPost["oper"] == "save" ? RequestPost[_n] : (_act != "add" ? (_act == "copy" && !fld.Edit.Enable ? "" : ((DateTime)dt.Rows[0][fld.Data.FieldName]).ToShortDateString()) : ""));
                    ...
                    //<br/> Уведомления:<br/>
                    _ret += " <div class=\"panel panel-default\" style=\"margin-top: 15px;\">";
                    _ret += "<div class=\"panel-body\">";
                    JSReadyList.Add("hidesigninput", "$('#_complect_repair_id_sign').parent().hide();");

                    if (_act == "add" || _act == "copy" || !cur.EnableSaveButton)
                    {
                        _ret += "<button type=\"button\" class=\"btn btn-md btn-default\" style=\"width:290px;\" disabled >Подпись исполнителем</button><br/><br/>";
                    }
                    else
                    {
                        ...
                        if (signed)
                        {
                            _ret += "<button id=\"scanbutton\" type=\"button\" title=\"Подпись исполнителем, посредством сканера шрих-кодов\" class=\"btn btn-md btn-success\"" +
                                " style=\"width:290px;white-space: normal;\">Подписано:" + dt.Rows[0]["id_sign_name_text"].ToString() + "</button><br/><br/>";
                            //JSReadyList.Add("disableinput", "$('#cph__complect_repair_id_perf').prop('disabled',true);");
                            JSReadyList.Add("disableinput", "$('#rrr').find('input,button').prop('disabled',true); $('#oper').prop('disabled',false);$('#btnDel,#savebutton').prop('disabled',true);$('#btnDel,#savebutton').attr('onclick','');");
                        }
                        else
                        {
                            _ret += "<button id=\"scanbutton\" type=\"button\" title=\"Подпись исполнителем, посредством сканера шрих-кодов\" class=\"btn btn-md btn-default\"" +
                            " style=\"width:290px;white-space: normal;\" " +
                            "onclick=\"if($(this).val()=='wait'){$(this).val('sleep');unbind_event();}else{$(this).val('wait');bind_event();}$(this).blur();\"" +//<img src=\\'/styles/images/loading.gif\\'>
                        ...
                        JSFunctionList.Add("bind_event()",
                         ...
                         "                  $.ajax({url: '/ajax/get_user_by_barcode.aspx?&barcode=' + escape(barcode),type: 'POST'," +
                                                    "success: function (html) {" +
                                                        ...
                                                        "else{if (confirm('Подписать исполнителем - ' + arr[1] + '?')){$('#_complect_repair_id_sign').val(arr[0]);$('#cph__complect_repair_id_sign').val(arr[1]);$('#_complect_repair_id_perf').val(arr[0]);$('#cph__complect_repair_id_perf').val(arr[1]);unbind_event();$('#savebutton').click();}}}});" +
                         // "                alert(barcode);" +
                         "            }" +
                         "            chars = [];" +
                         "            pressed = false;" +
                    ...
                    /*
                    JSReadyList.Add("scanerdiv",

               */

                    if (_act == "add" || _act == "copy" || !cur.EnableSaveButton)
                    {
                        _ret += "<button type=\"button\" title=\"Уведомление исполнителя\" class=\"btn btn-md btn-default\" style=\"width:290px;margin-bottom: 5px;\" disabled>" +
                            "Отправить исполнителю</button>";
                    }
                    else
                    {
                        bool pm = (bool)dt.Rows[0]["perf_mail"];

                        _ret += "<button type=\"button\" title=\"Уведомление исполнителя о создании комплекта движения документов\" class=\"btn btn-md btn-" +
                            (pm ? "success" : "default") + "\" style=\"width:290px;margin-bottom: 5px;\" onclick=\"SendMailGroup('pm');\">" +
                            (pm ? "Отправлено" : "Отправить") + " исполнителю</button>";

                        JSFunctionList.Add("SendMailGroup(gr)",
                        "  if (confirm('Отправить уведомление?')) {" +
                        "$.ajax({" +
                               "   url: '/ajax/SendMailComplectRepair.aspx?ru='+escape('" + HttpContext.Current.Request.Url + "')+'&id=" +
                    ...
                    if (CursorCount > 0)
                    {
                        ...
                        foreach (string k in list)
                        {
                            ...
                            JSReadyList.Add("form" + k + "dialog_nextbuttonbind",
                                ...
                                "var idlist=$('#" + k + "_id').val();" +
                                "if(keycode == '13'){" +
                                "   $.ajax({" +
                                "   url: '/ajax/InsertFromScanerRepair.aspx?id=" + _id + "&cur=" + k + "_cursor_" + _id + "&idlist='+idlist," +
                                "   type: 'POST'," +
                                "       data: $('#" + k + "_barcode').val()," +
                                "       cache: false," +
                                "       contentType: false," +
                                "       processData: false," +
                                "       success: function (html) { if (html.length>0){alert(html);} else{ if (idlist=='')NavigateNew(); else NavigateNext($('#" + k + "_barcode').val());}}," +
                                "       error: function (request, status, error) {  alert(request.responseText); }" +
                                "   });" +
                                "}});");
                            ...
                            JSFunctionList.Add("NavigateNew()",
                                "jQuery('#cph_jqGrid_complect_repair_list').trigger('reloadGrid');" +
                                "$('#form" + k + "_edit').dialog('close');" +
                                "AddRow" + k + "();");
                    ...
                    if (ActionMenuItems.Count > 0)
                    {
                        ...
                        _ret += "          <button type=\"button\" class=\"btn btn-default btn-xs dropdown-toggle\" data-toggle=\"dropdown\">";
                        _ret += "              " + (this.EditFormWidth > _maxwidth ? "&nbsp;Другое" : "") + "&nbsp;<span class=\"caret\"></span>&nbsp;";
                        _ret += "          </button>";
                        _ret += "          <ul class=\"dropdown-menu\" role=\"menu\">";
                    ...
                    _ret += "       </div>";
                    _ret += "       <div id=\"right_block\" style=\"float:right;width:" + (this.EditFormWidth > _maxwidth ? "204" : "80") + "px;\">";
                    _ret += "           <div class=\"btn-group\">";
                    _ret += "               <button id=\"savebutton\" type=\"button\" class=\"btn btn-xs btn-primary\" style=\"width: 100px;\" ";
                    ...
                    _ret += "           </div>&nbsp;";
                    _ret += this.EditFormWidth > _maxwidth ? "           <button type=\"button\" class=\"btn btn-xs btn-default\" style=\"width: 100px;\" onclick=\"window.onbeforeunload = null;$('form').find('#oper').val('close');$('form').submit();\">Закрыть</button>" : "";
        ...
        public void PrepareCursorJQGrid(string _cn, out JQGrid jqGrid)
        {
            ...
            if (cur.EnableSaveButton)
            {
                ...
                {//для вставки из буфера по текущей сортировке
                    HttpContext.Current.Session["_complect_repair_list_cursor_sidx"] = RequestGet["sidx"];
                    HttpContext.Current.Session["_complect_repair_list_cursor_sord"] = RequestGet["sord"];
                }
                ...
                _ret =
                    ...
                    "   var row = grid.getRowData(id);" +
                    "   $.ajax({" +
                    "   url: '/ajax/InsertFromCBRepair.aspx?id='+id+'&cur=" + _cn + "_cursor_" + _id + "'," +
                    "   type: 'POST'," +
                    "       data: buf," +
                    "       cache: false," +
                    "       contentType: false," +
            ...
            //        _ret += "       var f = '';";
            //        _ret += "       if (ida > 0 && row.file_archive!='') {f=row.file_archive;}else{f=row.file;}";
            //        _ret += "       if (f!='') window.open('/Archive/GetFile.aspx?id='+id +'&b=' + '" + BaseName + "'+'&f=' + f + '&k='+$.md5(id));";
            //        //}
            //        _ret += "       else alert('Файл не указан');";
            //        _ret += "   }";
            //        _ret += "   else alert('Выберите запись');  return false;";
            ...
            //                //"   }" +
            //                //"   f = f.substring(0, f.length - 1);" +
            //                "   window.open('/Archive/GetFileMulti.aspx?b=" + BaseName + "&cur=" + _cn + "_cursor_" + _id + "');" +//"&b=' + '" + BaseName + "'+'&f=' + escape(f));" +
            //                "}" +
            //                "else alert('Нет записей для выгрузки');  return false;");
            ...
            //                "   if (row.id_archive>0) window.open('/archive/" + BaseName + "/srch?id='+row.id_archive+'&act=view');else alert('Код ЭА не указан.');" +
            //                "}else alert('Выберите запись');  return false;";
            ...
            //                "    var f = row.file;" +
            //                "    var b = row.barcode;" +
            //                "    window.open('/archive/" + BaseName + "/srch?id=0&act=add&mode=complectnew&from='+id +'&b=' + b + '&f=' + f );" +
            //                "}else alert('Выберите запись');  return false;";
            //            JSFunctionList.Add("NewCard_" + _cn + "()", _ret);
            //        }
            //        #endregion
            ...
            //                "    var b = row.barcode;" +
            //                "    var id_archive=prompt('Введите Код ЭА',0);" +
            //                "    window.open('/archive/" + BaseName + "/srch?id='+id_archive+'&act=view&mode=complectnew&from='+id +'&b=' + b + '&f=' + f );" +
            //                "}else alert('Выберите запись');  return false;";
            //            JSFunctionList.Add("AddToCard_" + _cn + "()", _ret);
            //        }
            //        #endregion
                                ...
                                if (rid > 0)
                                {
                                    ...
                                    foreach (var fld in Cursors[k].Fields)
                                    {
                                        if (fld.Edit.Visible && fld.Edit.Enable)
                                        {
                                            ...
                                            if (fld.Edit.Auto != faAutoType.None && !fld.Edit.AddOnly)
                                        {
                                            cmd.CommandText += "[" + fld.Data.FieldName + "]=@p_" + fld.Data.FieldName + ",";
                                            _buf = "";
                                            switch (fld.Edit.Auto)
                                    ...
                                    foreach (var fld in Cursors[k].Fields.Skip(1))
                                    {
                                        if (fld.Edit.Control == faControl.File)
                                        {
                                            ...
                                            if (_mode == "complectnew" && row_new["from"] != null)
                                                row_new[fld.Data.FieldName] = System.IO.Path.Combine(Properties.Settings.Default.filepath, this.BaseName, "complect", row_new[fld.Data.FieldName].ToString());

                                            _rootpath = System.IO.Path.Combine(Properties.Settings.Default.filepath, this.BaseName, RouteName == "complectnew" ? "complect" : "archive");  //"\\\\stg.lan\\nfkdata\\ArchiveScanFiles\\"
